...
class FracAtlasDataset(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager: datasets.DownloadManager) -> List[datasets.SplitGenerator]:
      ...
      def save_shards(df, split_name):
        shard_dir = os.path.join(base_path, f"{split_name}_shards")
        os.makedirs(shard_dir, exist_ok=True)  # Create directory if not exists

        shards = np.array_split(df, num_shards)  # Split into `num_shards` parts
        shard_paths = []
        for i, shard in enumerate(shards):
            shard_path = os.path.join(shard_dir, f"{split_name}_shard_{i}.csv")
            shard.to_csv(shard_path, index=False)
            shard_paths.append(shard_path)
        return shard_paths

      train_shards = save_shards(train_df, "train")
      val_shards = save_shards(validation_df, "validation")
      test_shards = save_shards(test_df, "test")
    ...
    def _generate_examples(self, annotations_path, images_path, dataset_shards,localization_path):
        # logging.info("Generating examples from = %s", dataset_shards)
        ...
        for shard_path in dataset_shards:  # Process each shard
            logging.info(f"Processing shard: {shard_path}")
            shard_df = pd.read_csv(shard_path)  # Load shard
